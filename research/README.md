...
| `xsmom_bear.py` | Does a **deployable bear** cross-sectional edge exist? | Bear-specific plays (short liquid losers, reversal) **fail** — the bear short-edge lived in illiquid small-caps. But deployable **L/S at a shorter 4w lookback is all-weather** (bull 1.74 / bear 0.78 net of cost). So no separate bear strategy: choice is 4w-always-on vs 8w-longtilt+regime-gate. |
| `regime_xs.py` | **Marry XS-momentum with repo composite regime classifier** (steps 1+2) | **Checkpoint passed.** Market regime discriminates: XS-momentum dies in `trending_down_choppy` (weak bear, Sharpe −2.0), thrives in clean trends incl. clean downtrends. Gating off weak-bear → 4w config **1.08→1.80**, bear **−0.78→~0**. Validates strong/weak-bear taxonomy. Note: composite ranging states vestigial on 4h (default thresholds) → retune needed. |
| `regime_market.py` | Which market-regime **gate** is best (BTC vs universe vs breadth)? | **Breadth wins.** Universe-index composite ≈ BTC (same classifier). But a BREADTH gate (% of coins with +4w momentum) discriminates far better: high breadth → Sharpe **+3.2**, mid (transition) → **−1.8**, low → ~0. XS-momentum thrives on broad dispersion, dies in the mushy middle. Gate on breadth, set thresholds causally. **[CORRECTED by xsmom_validate: breadth FAILS causally — in-sample artifact.]** |
...
| `xsmom_validate.py` | **Step 3: causal + random-null + funding validation** | **Breadth gate FAILS causally** (−0.1σ vs random; the 3.2 was in-sample threshold fitting). Composite **weak-bear gate modestly survives** (+0.9σ, 0.72→0.87). Funding haircut real (~1.08→0.72). Validated deployable edge is **modest: ~0.85 Sharpe** market-neutral net of funding, NOT 1.8–3.2. |
| `xsmom_capacity.py` / `xsmom_hl.py` | **Deployability: realistic slippage + capacity (HL-native)** | **Deployable on Hyperliquid.** BinanceUS volumes were bad data (made it "die" at $50k); real HL liquidity → capacity to ~$20M, small slippage. Funding correctly SIGNED (longs pay/shorts receive) ≈ 0 net — Step 3's 0.85 was deflated by a one-sided funding error. Engine verified (reversal −3.16, random ~0). In-sample 2.78 on clean HL-large-caps; honest ~1.0–1.8 realistic. |
| `xsmom_trades.py` | **Deep trade-data analysis** — where the edge lives | Long leg = engine (fat tail, 46% win, +4.25%/2w); short = hedge (61% win, small). Signal concentrates in **top decile (+5.4%)**; absolute-bottom decile **bounces** (+0.7%) so the short is mis-targeted (skip it → ~2× short edge). **Concentration risk: 89% of P&L from 5 coins, FET alone 34%** → 2.78 is fragile, needs breadth + per-name caps. Signal slow (persists 1-4w). |
